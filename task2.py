...
class Model(object):
    ...
    def __init__(self):
        ...
        init_stddev = 1e-2
        state_size = 128  # RNN state vector size.
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('hidden'):
                init_state = tf.get_variable('init_state', [state_size], tf.float32, tf.random_normal_initializer(
                    stddev=init_stddev))  # Allow initial state to be a variable that is optimised with the other parameters
                batch_init = tf.tile(tf.reshape(init_state, [1, state_size]),
                                     [batch_size, 1])  # Replicate the initial state for every item in the batch
                cell = tf.contrib.rnn.BasicRNNCell(state_size)
                #cell = Cell(image_width, state_size, init_stddev)
            ...
            self.error = tf.reduce_mean(
                tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.targets, logits=logits))
